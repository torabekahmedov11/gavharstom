...
export let tgBot: Bot | null = null;

export const startBot = (prisma: PrismaClient) => {
  ...
  bot.command('start', async (ctx) => {
    ...
    if (!patient || !patient.phoneNumber) {
      ...
      await ctx.reply(
        `Assalomu alaykum, ${firstName}!\n\nKlinikamizga xush kelibsiz. Navbat olish uchun iltimos, pastdagi tugmani bosib telefon raqamingizni yuboring. Bu ma'lumot administratorimiz siz bilan bog'lanishi va navbatni 100% tasdiqlashi uchun zarur.`,
...
function showWebAppButton(ctx: any) {
